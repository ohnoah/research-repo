#!/usr/bin/env python3
"""
End-to-end smoke test across all PPTX templates in `examples/ClearView`.

For each PPTX (slide 0):
- Discover chart locators (Aspose)
- Build prompt payload (DSL or workbook snapshot)
- Create synthetic "LLM output" update
- Apply update to produce output PPTX files

Outputs are written under:
  examples/hybrid_pipeline_test_runs/_artifacts/<run_id>/smoke_clearview/...

Per-template folder contains:
- source_original.pptx
- updated_after_chart_XX.pptx (cumulative)
- updated_after_all_charts.pptx (copy of the final cumulative output)
"""
...
def main() -> int:
    ...
    out_root = ARTIFACTS_ROOT / run_id / "smoke_clearview"
    out_root.mkdir(parents=True, exist_ok=True)

    clearview_root = REPO_ROOT / "examples" / "ClearView"
    pptxs = sorted(clearview_root.rglob("*.pptx"))
    ...
    for src in pptxs:
        ...
        try:
            locs = h.list_chart_locators_aspose(str(source_original), 0)
            if not locs:
                lines.append(f"SKIP no charts: {rel}")
                continue

            current = str(source_original)
    ...
    readme = out_root.parent / "README.txt"
    readme.write_text(
        "\n".join(
            [
                "ClearView smoke run",
                ...
                "",
                "Log:",
                f"- {out_root.relative_to(REPO_ROOT)}/smoke_log.txt",
                "",
            ]
        ),
        encoding="utf-8",
