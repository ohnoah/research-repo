...
Use gateway mode to route read-only commands through the agent gateway.

```bash
# Store gateway credentials in the config
hatchet config init --mode gateway --gateway-url https://<gateway>.vercel.app --gateway-token <token>

# Or set env vars
export HATCHET_MODE=gateway
export GATEWAY_URL=https://<gateway>.vercel.app
export GATEWAY_TOKEN=<token>

# Read-only commands
hatchet workflows list --mode gateway
hatchet runs list --mode gateway
hatchet runs get <run-id> --mode gateway
hatchet steps logs <step-id> --mode gateway
```
